package test.com.huercha.api;

import com.alibaba.fastjson.JSON;
import com.huercha.api.beans.ApiDataBean;
import com.huercha.api.configs.ApiConfig;
import com.huercha.api.excepions.ErrorRespStatusException;
import com.huercha.api.listeners.AutoTestListener;
import com.huercha.api.listeners.RetryListener;
import com.huercha.api.utils.*;
...
public class ApiTest extends TestBase {
	...
	/**
	 * 是否使用form-data传参会在post与put方法封装请求参数用到
	 */
...
//						.toString(respEntity));
	...
	private HttpUriRequest parseHttpRequest(String url, String method, String param) throws UnsupportedEncodingException {
		//处理url
		url = parseUrl(url);
		ReportUtil.log("请求方法：" + method);
		ReportUtil.log("请求URL：" + url);
		ReportUtil.log("传递参数：" + param.replace("\r\n", "").replace("\n", ""));
		...
		if ("post".equalsIgnoreCase(method) || "upload".equalsIgnoreCase(method)) {
			...
			//如果请求头的content-type的值包含form-data或者请求方法为upload(上传)时采用MultipartEntity形式
